...
module.exports = class Network extends EventEmitter {
  ...
  start() {
    ...
    this.middleware.post('/api', bodyParser.json(), this.handleAPIRequest.bind(this));
  ...
  handleAPIRequest(req, res){
    ...
    if (!req.body || !req.body.action || !this.apiRequestHandlers[req.body.action]) return res.status(404).send('bad or missing action in body');

    this.apiRequestHandlers[req.body.action](req, res);
  }
